...
const PROJECTS = {
  ...
  'sears-catalog': {
    ...
    blurb: 'This adaptive reuse project looks at how to take an existing building and turn it into a new one. This was done through the tagging of each of the pieces in a Sears catalog house, which were then taken in chunks through a process of automation where the frustum of cameras frame the chunks that are taken from the original. Using security camera logic, cameras were placed throughout the house to begin the process. Using the chunks taken from the original Norwood House, they were then reorganized into a new form that suited how we understood a modern single family home. It was then re-covered with remaining materials left over from the tagging process in the beginning as well as a new roof system to finish the project.',
  ...
  'combinatory-form': {
    ...
    blurb: 'This series of objects displays the way Grasshopper can be used as a generative design tool through the combination of systems. These systems, site, object, and line, are all used in conjunction to create an infinite number of computer generated combinatory forms. These infinite forms show signs of logical categorization. By far the most common group is fragmentation of the primitive, often creating islands and almost unrecognizable shards. The next group is more effective at showing combinatory form, whereas the objects are shaped more by the site than by other nested objects. The density of objects is important to note as well due to the overlap and melting of the objects together to create a more unified whole. The model is a continuation of the communication of these ideas calling out the carved surface from the site with brushed graphite, while the innermost nested regions are left in their natural rough state showing the difference between the nested and cut elements.',
  ...
  'hippotherapy': {
    ...
    blurb: 'Hippotherapy is the use of a horse’s gait to mimic the movement of the human pelvis to speed up recovery. This project is an extension of this therapeutic idea through mimicking the horse’s gait organizationally and formally. The plan was separated logically into its programmatic functions with three wings in the front, medical therapy, psychological therapy, and admin functional spaces. The large connecting space in the back functions as the hippotherapy arena where sessions take place. Organizationally the horse’s gait is where the circulation spaces occur separating the building into standard and eccentric spaces. The forms were all derived from various combinations of both sides of the horse’s gait being lofted together into unique planes which follow the organizational cuts happening in the project.',
